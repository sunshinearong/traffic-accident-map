...
def normalize_message(msg, conversions):
    ...
    #my answere
    # if a token appears as a key in the conversions` dictionary
    # then replace it with the corresponding dictionary value.
    for i in range(len(tokens)):
        if tokens[i] in conversions.keys():
            tokens[i] =conversions[tokens[i]]
...
def split_tweet(tweet):
    ...
    # date_and_time = None   # FIXME: Currently sets data_and_time to None
    #message = ""           # FIXME: Currently sets message to the empty string

    #my answere
    tweet_split = tweet.split(" ")  #split tweet with whitespace
    date_and_time = " ".join(tweet_split[1:3])  #get the second and third items of tweet_split:date_and_time
    message=" ".join(tweet_split[5:])   #message:from the six item to the end
...
def match_streets(tweet_msg, street_locations):
    ...
    for key in street_locations.keys():
        if tweet_msg.find(" ".join(key)) != -1: #if tweet_msg contains key ,return key
            return [key]
...
def suburb_histogram(tweets_filename, suburb_filename):
    ...
    with open(tweets_filename) as fh:
        ...
        for li in fh.readlines():   #process file line by line
            for key in histogram.keys():
                histogram[key] += li.count(key) #compute how many key each line has,update histogram[key]
    return histogram
