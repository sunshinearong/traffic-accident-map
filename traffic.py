...
def read_street_locations(filename):
    ...
    for line in fh:
        ...
        # `query_name`. Do not include the state and make sure you
        # remove whitespace from around the text. Also extract the
        # latitude and longitude as floats. Add a key-value pair to
        # the `street_locations` dictionary where the key is a tuple
        # of street name and suburb, and the value is a tuple of
        # latitude and longitude coordinates.
        #print(query_name.split(','))
        street = query_name.split(',')[0]   #split query by ',',the first item is street name
        suburb=' '.join(query_name.split(" ")[:-1]).split(",")[-1].strip()  #between ',' and the last word is suburb name,strip():drop whitespace
        street_locations[(street,suburb)]=(eval(lat),eval(lon))#eval():convert str object to float type
    fh.close()
...
def read_map(resolution):
    ...
    limits = {}
    limit=[]#list of data from the file
    with open("data/canberraMapLatLon_" + resolution + ".txt") as f:
        for li in f.readlines():#process file line by line
            for d in li.strip('\n').split(' '):#drop '\n'and split line by whitespace
                limit.append(eval(d))#add d to limit,eval():convert str object to float type
    limits['lat_max'] = limit[0]#the first item in limit is lat_max
    limits['lat_min'] = limit[1]#same as above
...
def animate_tweets(tweet, street_locations, map_image, map_limits):
    ...
    date_time = tu.split_tweet(tweet)[0][2:]   #apply split_tweet in tu for getting date_time and msg
